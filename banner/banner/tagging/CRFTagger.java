...
public class CRFTagger implements Tagger
{
    ...
    // own implementation
    private static void setupPipesModified(ArrayList<Pipe> pipes)
    {
        pipes.add(new RegexMatches("ALPHA", Pattern.compile("[A-Za-z]+")));
        pipes.add(new RegexMatches("INITCAPS", Pattern.compile("[A-Z].*")));
        pipes.add(new RegexMatches("UPPER-LOWER", Pattern.compile("[A-Z][a-z].*")));
    ...
    // own implementation
    public static CRFTagger train(List<Sentence> sentences, List<Sentence> testSentences, int order,
            ...
            dragon.nlp.tool.Tagger posTagger, boolean useNumericalNormalization)
    {
        ...
        if (textDirection.doForward())
            forwardCRF = train(sentences, testSentences, order, useFeatureInduction, format,
                    pipe, false);
        CRF4 reverseCRF = null;
        if (textDirection.doReverse())
            reverseCRF = train(sentences, testSentences, order, useFeatureInduction, format,
                    pipe, true);
    ...
    // own implementation
    public void evaluateModel(List<Sentence> sentences)
    {
        String2TokenSequencePipe localBasePipe = new String2TokenSequencePipe(null, null, false);
        ArrayList<Pipe> pipes = new ArrayList<Pipe>();
        pipes.add(localBasePipe);
        setupPipesModified(pipes);
        Pipe pipe = new SerialPipes(pipes);
        ...
        {
            String text = sentence.getTrainingText(TagFormat.IOB, false);
    ...
    // own implementation
    private static CRF4 train(List<Sentence> sentences, List<Sentence> testSentences, int order, boolean useFeatureInduction, TagFormat format, Pipe pipe, boolean reverse)
    {
        InstanceList instances = new InstanceList(pipe);
        for (Sentence sentence : sentences)
        {
        ...
        {
            String text = sentence.getTrainingText(format, reverse);
            testInstances.add(new Instance(text, null, sentence.getTag(), null, pipe));
        }
